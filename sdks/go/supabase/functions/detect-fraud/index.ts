...
serve(async (req) => {
  ...
  try {
    ...
    switch (transaction_type) {
      case 'trial_signup':
        riskAnalysis = await handleTrialSignup(supabaseClient, data);
        break;
        
      case 'referral':
        riskAnalysis = await handleReferral(supabaseClient, data);
        break;
    }
    
    // Create fraud alert if needed
    await createFraudAlert(supabaseClient, transaction_type, riskAnalysis.risk_score, data);
